...
void AutoTele::OnLoad() {
	...
	new Checkhook(settingsTab, 4, 27, &Toggles["Display Messages"].state, "Display Messages");
	...
	new Checkhook(settingsTab, 4, 57, &Toggles["Draw Destination"].state, "Draw Destination");
	new Keyhook(settingsTab, 150, 57, &Toggles["Draw Destination"].toggle, "");

	new Checkhook(settingsTab, 4, 72, &Toggles["CP to cave"].state, "CP to cave");
	new Keyhook(settingsTab, 150, 72, &Toggles["CP to cave"].toggle, "");

	new Texthook(settingsTab, 60, 92, "Hotkeys");

	new Keyhook(settingsTab, 4, 107, &NextKey, "Next Tele");

	new Keyhook(settingsTab, 4, 122, &OtherKey, "Other Tele");

	new Keyhook(settingsTab, 4, 137, &WPKey, "WP Tele");

	new Keyhook(settingsTab, 4, 152, &PrevKey, "Prev Tele");
...
void AutoTele::OnLoop() {
	...
	if(LastArea != GetPlayerArea() && D2CLIENT_GetPlayerUnit()) {

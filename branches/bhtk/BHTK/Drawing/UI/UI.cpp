...
UI::UI(std::wstring name, unsigned int xSize, unsigned int ySize) {
	...
	SetYSize(ySize);
	SetName(name);
	wstring path = BHTK::path + L"UI.ini";
	int x = GetPrivateProfileInt(name.c_str(), L"X", 0, path.c_str());
	SetX(x);
	int y = GetPrivateProfileInt(name.c_str(), L"Y", 0, path.c_str());
	SetY(y);
	wchar_t activeStr[20];
	GetPrivateProfileString(name.c_str(), L"Minimized", L"true", activeStr, 20, path.c_str());
...
UI::~UI() {
	Lock();
	wstring path = BHTK::path;
	WritePrivateProfileString(name.c_str(), L"X", to_string<unsigned int>(GetX()).c_str(), wstring(path + L"UI.ini").c_str());
	WritePrivateProfileString(name.c_str(), L"Y", to_string<unsigned int>(GetY()).c_str(), wstring(path + L"UI.ini").c_str());
	WritePrivateProfileString(name.c_str(), L"Minimized", to_string<bool>(IsMinimized()).c_str(), wstring(path + L"UI.ini").c_str());

...
Combohook::Combohook(HookVisibility visibility, unsigned int x, unsigned int y, unsigned int xSize, unsigned int* index, std::vector<std::wstring> opts)
	: Hook(visibility, x, y) {
		SetXSize(xSize);
		currentIndex = index;
		options = opts;
...
Combohook::Combohook(HookGroup* group, unsigned int x, unsigned int y, unsigned int xSize, unsigned int* index, std::vector<std::wstring> opts)
	: Hook(group, x, y) {
		SetXSize(xSize);
		currentIndex = index;
		options = opts;
...
bool Combohook::OnLeftClick(bool up, unsigned int x, unsigned int y) {
	...
	if (active && x >= GetX() && y >= GetY() && x <= GetX() + GetXSize() && y <= GetY() + ((options.size() + 1) * (GetYSize() + 4))) {
		int n = 0;
		for (vector<wstring>::iterator it = options.begin(); it < options.end(); it++,n++) {
...
void Combohook::OnDraw() {
	...
	Texthook::Draw(GetX() + GetXSize() - 8, GetY() + 3, 0, GetFont(), InHook((*p_D2CLIENT_MouseX), (*p_D2CLIENT_MouseY))||active?Tan:Gold, L"v");

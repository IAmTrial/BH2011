...
Inputhook::Inputhook(HookVisibility visibility, unsigned int x, unsigned int y, unsigned int xSize, std::wstring formatString, ...) :
 Hook(visibility, x, y) {
	SetXSize(xSize);
	SetFont(0);
	SetActive(false);
...
Inputhook::Inputhook(HookGroup* group, unsigned int x, unsigned int y, unsigned int xSize, std::wstring formatString, ...) :
 Hook(group, x, y) {
	SetXSize(xSize);
	SetFont(0);
	SetActive(false);
 ...
 void Inputhook::SetText(wstring newText, ...) {
	wchar_t buffer[4096];
	va_list arg;
...
unsigned int Inputhook::GetCharacterLimit() {
	return (GetXSize() / Texthook::GetTextSize(L"A", GetFont()).x);
}
 ...
 void Inputhook::OnDraw() {
	 ...
	 wstring drawnText = text;

	 //Draw the text in!
	 unsigned int len = drawnText.length() - textPos;
	...
	drawnText = drawnText.substr(textPos, len);
	 ...
	 if (IsSelected()) {
		 drawnText.insert(GetSelectionPosition() + GetSelectionLength(), L"ÿc0");
		 drawnText.insert(GetSelectionPosition(), L"ÿc9");
	 }

	 Texthook::Draw(GetX() + 3, GetY() + 3, None, GetFont(), White, drawnText);
		...
		default:
			...
			if (ctrlState) {
				...
				if (key == 0x43 || key == 0x58) {
					...
					Lock();
					wstring mText = text.substr(GetSelectionPosition(), GetSelectionLength());
			
					HGLOBAL hGlobal = GlobalAlloc(GMEM_MOVEABLE, (mText.size() + 1) * sizeof(wchar_t)); 
					wchar_t* szStr = (wchar_t*)GlobalLock(hGlobal);
					memcpy(szStr, mText.c_str(), mText.size() * sizeof(wchar_t));
			...
			if (ToAscii(key, (lParam & 0xFF0000), layout, out, 0) == 0)
				return false;
			swprintf_s(wChar, sizeof(wChar), L"%c", out[0]);
 ...
 void Inputhook::InputText(wstring newText) {
	 Lock();
...
void Inputhook::Replace(unsigned int pos, unsigned int len, std::wstring str) {

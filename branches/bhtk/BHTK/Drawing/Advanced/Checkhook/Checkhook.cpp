 ...
 */
Checkhook::Checkhook(HookVisibility visibility, unsigned int x, unsigned int y, bool* checked, std::wstring formatString, ...) :
Hook(visibility, x, y) {
 ...
 */
Checkhook::Checkhook(HookGroup* group, unsigned int x, unsigned int y, bool* checked, std::wstring formatString, ...) :
Hook(group, x, y) {
	//Correctly format the string from the given arguments.
	SetTextColor(Gold);
	state = checked;
 ...
 */
void Checkhook::SetText(std::wstring formatString, ...) {
	wchar_t buffer[4096];
	va_list arg;
	va_start(arg, formatString);
	vswprintf_s(buffer, 4096, formatString.c_str(), arg);
...
unsigned int Checkhook::GetXSize() {
	return Texthook::GetTextSize(text, 0).x + 20; 
}
